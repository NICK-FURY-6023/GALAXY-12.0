                ...
                try:
                    ...
                    if data["identifier"] != "LOCAL":
                        print(f'⚠️ - Server verification failed [{data["identifier"]}], '
                              f'new attempt [{retries}/{max_retries}] em {backoff} segundos.')
                    backoff += 2
                    retries += 1
        ...
        for skin in os.listdir("./utils/music/skins/normal_player"):
            ...
            try:
                ...
                if not hasattr(skin_file, "load"):
                    print(f"Skin ignored: {skin}.py | Função load() não configurada/encontrada...")
                    continue
    ...
    def setup(self):
        ...
        if self.config["AUTO_DOWNLOAD_LAVALINK_SERVERLIST"]:
            ...
            try:
                ...
                print(f"Downloading lavalink server list (arquivo: {ini_file})")
                try:
        ...
        for key, value in self.config.items():
            ...
            if key.lower().startswith("lavalink_node_"):
                try:
                    ...
                    print(f"Failure when adding node: {key}, erro: {repr(e)}")
        ...
        if mongo_key:
            ...
            print("Database em uso: TinyMongo | Note: Database files will be saved locally in the folder: local_database")
    ...
    async def update_appinfo(self):
        ...
        if self.appinfo.bot_public and not self.config.get("SILENT_PUBLICBOT_WARNING"):
            ...
            warn_msg = f"Attention: bot [{self.user}] (ID: {self.user.id}) as been configured in the developer portal " \
                  ...
                  "https://github.com/NICK-FURY-6023/GALAXY10.O.O/blob/main/LICENSE\n" \
            ...
            if guilds:
                ...
                if (gcount:=len(guilds)) > 10:
                    warn_msg += F"\ne em mais {gcount-10} more server (s)."
